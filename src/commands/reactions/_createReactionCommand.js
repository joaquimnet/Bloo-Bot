...
module.exports = options => {
  return async function (message, args, call) {
    ...
    const msgTarget = options.msgTarget.replace(/%user/gi, `<@${call.caller}>`).replace(/%target/gi, target);
    const msgNoTarget = options.msgNoTarget.replace(/%user/gi, `<@${call.caller}>`);
